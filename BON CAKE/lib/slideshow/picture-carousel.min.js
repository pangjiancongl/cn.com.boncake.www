  ...
  PictureCarousel.prototype = {
    init: function () {
      ...
      container_width = parseInt(
        container_width.substring(0, container_width.length - 2)
      );
      container_height = parseInt(
        container_height.substring(0, container_height.length - 2)
      );
    ...
    generateHorizontalCarousel: function (containerWidth, containerHeight) {
      ...
      for (var i = 0; i < this.options.listData.length; i++) {
        container_h_li +=
          ...
          li_height +
          'px;"><img src="' +
          this.options.listData[i].src +
          '" alt="' +
          this.options.listData[i].alt +
          '" /></li>';
      }
      var container_h =
        '<div class="horizontal-carousel"><ul>' +
        container_h_li +
        container_h_li +
        "</ul></div>";
      ...
      if (this.options.arrow) {
        var leftBtn = this.parseElement(
          '<div class="horizontal-carousel-arrow-left" ></div>'
        );
        var rightBtn = this.parseElement(
          '<div class="horizontal-carousel-arrow-right" ></div>'
        );
        this.el.childNodes[0].appendChild(leftBtn);
        this.el.childNodes[0].appendChild(rightBtn);
        this.addEvent(this.el.childNodes[0], "mouseenter", function () {
        ...
        this.addEvent(this.el.childNodes[0], "mouseleave", function () {
        ...
        this.addEvent(leftBtn, "click", function () {
        ...
        this.addEvent(rightBtn, "click", function () {
          if (that.arrowClickable) {
            that.arrowClickable = false;
            that.scroll2Right(li_width);
            setTimeout(function () {
      ...
      if (this.options.dots) {
        ...
        var dotsContainer = this.parseElement(
          '<div class="carousel-indicator-dots"><ul>' + dots + "</ul></div>"
        );
        this.el.childNodes[0].appendChild(dotsContainer);
        this.dots();
        var dot_sarr = this.el.childNodes[0].lastChild.childNodes[0].childNodes;
        for (var a = 0; a < dot_sarr.length; a++) {
          (function (a) {
            that.addEvent(dot_sarr[a], "click", function () {
    ...
    dots: function () {
      var dots_ul = this.el.querySelector(".carousel-indicator-dots")
        .childNodes[0];
